...
def get_cut_line(AlignList, line: str, sta: float, length: 'float > 0' = 100.0):
    """
    获取指定路线指定桩号切线（长度默认为 100m）
    :param AlignList: ei 数据字典
    :param length: 切线长度
    :param line: 路线名称
    :param sta: 桩号
    :return: 起点、终点坐标
    """
    line = line.upper()
    cord1 = AlignList[line].curPQX.GetCoord(sta)    # 主线坐标
    cord2 = AlignList[line].curPQX.GetDir(sta)      # 主线切线坐标

    cord1 = [i for i in cord1]
    start = [cord1[0] - cord2[1] * length / 2, cord1[1] + cord2[0] * length / 2]  # 主线法线坐标 1
    end = [cord1[0] + cord2[1] * length / 2, cord1[1] - cord2[0] * length / 2]  # 主线法线坐标 2
