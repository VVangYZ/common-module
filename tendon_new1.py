...
class Tendon:
    ...
    def __init__(self, one_sec):
        ...
        self.top = self.center_y + one_sec.prop['c'][2]  # 上缘坐标
        self.bot = self.center_y - one_sec.prop['c'][3]  # 下缘坐标
    ...
    def get_np_from_pw(
        ...
        while True:
            ...
            if np_min < np_test < np_max:
                ...
                pro_offset = ep_good - np.average(pro_ep, weights=pro_n)     # 偏心距改变
                pro_ep_0 = pro_offset + pro_ep[0]           # 第一排钢束改变后偏心值
                if abs(pro_ep_0) > abs(ep0):                # 第一排不能超出边界
                    pro_offset = ep0 - pro_ep[0]
                pro_ep = [i + pro_offset for i in pro_ep]   # 修正
